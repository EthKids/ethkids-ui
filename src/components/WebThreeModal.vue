<template>
  <modal v-if="showModal"
    ...
    <div slot="body">
      <div class="row mt-3">
        <div class="col provider" v-on:click="connectMetamask()">
          <img style="max-width: 150px;height: 100px;"
        ...
        <div class="col provider" v-on:click="connectWalletConnect()">
          <img style="max-width: 150px;height: 100px;"
...
export default {
  ...
  beforeCreate() {
    const self = this;
    EventBus.subscribe('SHOW_WEB3_MODAL', (data) => {
      self.showModal = true;
  ...
  methods: {
    ...
    async connectWalletConnect() {
      ...
      const provider = new WalletConnectProvider({
        infuraId: this.$store.state.infuraId
      });
    ...
    async commitWeb3(web3, provider) {
      const coinbase = (await web3.eth.getAccounts())[0];
      const networkId = await web3.eth.net.getId();
      this.$store.commit('disableReadOnlyMode');
      //new we3
      ...
      provider.on("accountsChanged", (accounts) => {
        ...
        this.$store.commit('registerCoinbase', accounts[0]);
        this.$store.commit('financialChange');
      });
      provider.on("chainChanged", (chainId) => {
        console.log('chainChanged' + chainId);
        this.$store.dispatch('instantiateWeb3ReadOnly')
        this.$store.commit('financialChange');
      });
...
.provider:hover {
  ...
  box-shadow: 0 0 15px #e9e6e6, 0 0 15px #eeecec, 0 0 15px #f5f3f3;
}
