...
export default new Vuex.Store({
  state: {
    registryAddress: '0x95819E2710E1Cc238fA69b2c9f44B8ADEc994844',
    web3: {
      isInjected: false,
      web3Instance: null,
      networkId: null,
    ...
    initCharityVaultContract({commit, dispatch}, charityVaultAddress) {
      ...
      getCharityVaultContract(charityVaultAddress).then((charityVaultContract) => {
        ...
        charityVaultContract.methods.sumStats().call().then((sumRaised) => {
          commit('registerTotalDonationsRaised', sumRaised);
        });

        window.web3.eth.getBalance(charityVaultAddress, (err, charityBalance) => {
          commit('registerCharityVaultBalance', charityBalance);
        });
